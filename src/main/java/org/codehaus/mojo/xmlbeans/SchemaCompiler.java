...
public class SchemaCompiler
{
    ...
    private static SchemaTypeSystem loadTypeSystem( String name, File[] xsdFiles, File[] wsdlFiles, File[] configFiles,
                                                    ResourceLoader cpResourceLoader, boolean download, boolean noUpa,
                                                    boolean noPvr, Set mdefNamespaces, File baseDir,
                                                    Map sourcesToCopyMap, Collection outerErrorListener,
        ...
        if ( xsdFiles != null )
        {
            for ( int i = 0; i < xsdFiles.length; i++ )
            {
                try
                {
                    ...
                    {
                        StscState.addError( errorListener, "Document " + xsdFiles[i] + " is not a schema file",
                                            XmlErrorContext.CANNOT_LOAD_XSD_FILE, schemadoc );
                    }
                    else
                    {
                ...
                {
                    StscState.addError( errorListener, "Cannot load file " + xsdFiles[i] + ": " + e,
        ...
        if ( wsdlFiles != null )
        {
            for ( int i = 0; i < wsdlFiles.length; i++ )
            {
                try
                {
                    ...
                    {
                        StscState.addError( errorListener, "Document " + wsdlFiles[i] + " is not a wsdl file",
                                            XmlErrorContext.CANNOT_LOAD_XSD_FILE, wsdldoc );
                    }
                    else
                    {
                        if ( wsdlContainsEncoded( wsdldoc ) )
                        {
                            StscState.addWarning( errorListener,
                                                  "The WSDL " + wsdlFiles[i] +
                                                  " uses SOAP encoding. SOAP encoding is not compatible with literal XML Schema.",
                                                  XmlErrorContext.CANNOT_LOAD_XSD_FILE, wsdldoc );
                        }
                        ...
                        for ( int j = 0; j < types.length; j++ )
                        {
                            ...
                            SchemaDocument.Schema[] schemas = (SchemaDocument.Schema[]) types[j].selectPath(
                                "declare namespace xs=\"http://www.w3.org/2001/XMLSchema\" xs:schema" );
                        ...
                        StscState.addInfo( errorListener,
                                           "Processing " + count + " schema(s) in " + wsdlFiles[i].toString() );
                    }
                }
                catch ( XmlException e )
                {
                ...
                {
                    StscState.addError( errorListener, "Cannot load file " + wsdlFiles[i] + ": " + e,
                                        XmlErrorContext.CANNOT_LOAD_XSD_FILE, wsdlFiles[i] );
                }
            }
        }

        SchemaDocument.Schema[] sdocs = (SchemaDocument.Schema[]) scontentlist.toArray(
        ...
        if ( configFiles != null )
        {
            for ( int i = 0; i < configFiles.length; i++ )
            {
                try
                {
                    ...
                    {
                        StscState.addError( errorListener, "Document " + configFiles[i] + " is not an xsd config file",
                ...
                {
                    StscState.addError( errorListener, "Cannot load xsd config file " + configFiles[i] + ": " + e,
                                        XmlErrorContext.CANNOT_LOAD_XSD_CONFIG_FILE, configFiles[i] );
                }
            }
        }
        ConfigDocument.Config[] cdocs = (ConfigDocument.Config[]) cdoclist.toArray(
            new ConfigDocument.Config[cdoclist.size()] );
    ...
    public static boolean compile( SchemaCompiler.Parameters params )
    {
        ...
        XmlErrorWatcher errorListener = new XmlErrorWatcher( outerErrorListener );
        SchemaTypeSystem system = loadTypeSystem( name, xsdFiles, wsdlFiles, configFiles, cpResourceLoader, download,
                                                  noUpa, noPvr, mdefNamespaces, baseDir, sourcesToCopyMap,
                                                  errorListener, entityResolver );
        ...
        {
            System.out.println(
                "Time to build schema type system: " + ( (double) ( finish - start ) / 1000.0 ) + " seconds" );
        }
        ...
        if ( result && system != null ) // todo: don't check "result" here if we want to compile anyway, ignoring invalid schemas
        {
            ...
            result &=
                SchemaCodeGenerator.compileTypeSystem( system, srcDir, javaFiles, sourcesToCopyMap, classpath,
                                                       classesDir, outputJar, nojavac, jaxb, errorListener, repackage,
                                                       verbose, sourcefiles );
            ...
            if ( result )
            {
                ...
                {
                    System.out.println(
                        "Time to generate code: " + ( (double) ( finish - start ) / 1000.0 ) + " seconds" );
                }
            }
            ...
            if ( result && !nojavac )
            {
                ...
                if ( !CodeGenUtil.externalCompile( sourcefiles, classesDir, classpath, debug, compiler,
                ...
                {
                    System.out.println(
                        "Time to compile code: " + ( (double) ( finish - start ) / 1000.0 ) + " seconds" );
                }
    ...
    private static void runExtensions( List extensions, SchemaTypeSystem system, File classesDir )
    {
        if ( extensions != null && extensions.size() > 0 )
        {
            ...
            while ( i.hasNext() )
            {
                ...
                {
                    System.out.println(
                        "UNABLE to instantiate schema compiler extension:" + extension.getClassName().getName() );
                ...
                {
                    System.out.println(
                        "ILLEGAL ACCESS Exception when attempting to instantiate schema compiler extension: " +
